...
자연어 프롬프트 → MCP tool call 계약을 검증하는 테스트다.
1회 실행에 약 **$0.40 / 40초**가 소요되므로, 개발 중 반복 실행용이 아니다.

| 시점 | 실행 여부 | 이유 |
| --- | --- | --- |
| **PR 생성/업데이트** | O | CI에서 자동 실행, 결과를 PR 코멘트에 요약 |
| **도구 스키마·description 변경** | O | 프롬프트 ↔ tool call 계약이 깨지는지 확인 |
| **비즈니스 로직(API 응답 등) 변경** | X | 이 테스트 범위 밖 |
